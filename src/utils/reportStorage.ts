...
export interface SavedReport {
  ...
  settings: any;
  data: any;
  editable_rows?: any; // any[]에서 any로 변경
  created_at: string;
...
export const saveReport = async (reportData: {
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      editable_rows: reportData.editableRows || null // null로 기본값 설정
    }])
    .select()
    .single();
  ...
  return {
    ...
    data: data.data,
    editable_rows: data.editable_rows || undefined, // null을 undefined로 변환
...
export const getReports = async (): Promise<SavedReport[]> => {
  ...
  return data.map(report => ({
    ...
    data: report.data,
    editable_rows: report.editable_rows || undefined, // null을 undefined로 변환
